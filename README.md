...
**TKTK: Describe naughty-list here.**
...
```
cd naughty-list
mkvirtualenv --no-site-packages naughty-list
...
```
* * * * * ubuntu bash /home/ubuntu/apps/naughty_list/repository/cron.sh fab $DEPLOYMENT_TARGET cron_jobs.test 
```
...
|--------|------|-----|-----|--------|--------|
|naughty-list|tweet|`location`||||
|naughty-list|facebook|`location`||||
|naughty-list|email|`location`||||
|naughty-list|new-comment||||
|naughty-list|open-share-discuss||||
|naughty-list|close-share-discuss||||
|naughty-list|summary-copied||||
|naughty-list|featured-tweet-action|`action`||``tweet_url``|
|naughty-list|featured-facebook-action|`action`||``post_url``|
